...
def analyze_synthetic_data(invoices: list[dict]) -> None:
    ...
    total_value = 0
    line_items_count = 0
    fraud_count = 0  # Count invoices labeled as fraud
    ...
    for invoice in invoices:
        ...
        # Count fraud labels
        if invoice.get("label", 0) == 1:
            fraud_count += 1

    total_invoices = len(invoices)
    fraud_percentage = (fraud_count / total_invoices) * 100 if total_invoices > 0 else 0

    print(f"Analysis of {total_invoices} invoices:")
    print(f"- Unique merchants: {len(merchants)}")
    print(f"- Total line items: {line_items_count}")
    print(f"- Average line items per invoice: {line_items_count/total_invoices:.2f}")
    print(f"- Fraudulent invoices: {fraud_count} ({fraud_percentage:.2f}%)")
...
def load_dataset(filename):
    """
    Load the synthetic invoice dataset from a JSON file.
    """
...
def generate_dataset(num_invoices=100, num_merchants=1000, general_anomaly_rate = 0.3,
    line_anomaly_rate = 0.05, seed=42,filename=None):
    """
    ...
    invoices = generate_synthetic_invoice(num_invoices, num_merchants,general_anomaly_rate=general_anomaly_rate,line_anomaly_rate=line_anomaly_rate)
    analyze_synthetic_data(invoices,)
    if(filename):
        save_synthetic_data(invoices,base_filename=filename)
    else:
